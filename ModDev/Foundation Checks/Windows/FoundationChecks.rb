...
Dir.chdir(GAME_ROOT) do
  ...
  check("Expanded Dex runtime injection passes its isolated check") do
    system(
      RbConfig.ruby,
      File.join(GAME_ROOT, "ModDev", "Foundation Checks", "Windows", "ExpandedDexRuntimeCheck.rb"),
      :out => File::NULL,
      :err => File::NULL
    )
  end
  ...
  archive_process_fallback = begin
    ...
    command = [Reloaded::Platform.archive_tool_path, "i"]
    captured_output, captured_status = Reloaded::Archive.send(:capture_process, command)
    streamed_output = +""
    streamed_status = Reloaded::Archive.send(:stream_process, command) do |line, _pid|
      streamed_output << line
    end
    captured_status.success? && streamed_status.success? &&
      captured_output.include?("7-Zip") && streamed_output.include?("7-Zip")
  ensure
    Object.const_set(:Open3, archive_open3) if archive_open3 && !Object.const_defined?(:Open3)
  end
  check("Archive API runs bundled 7-Zip without Open3") { archive_process_fallback }
