...
module ReloadedMart
  module UI
    ...
    class << self
      ...
      def viewport_z
        (@viewport_z_override || DEFAULT_VIEWPORT_Z).to_i
      end

      def with_viewport_z(value)
        previous = @viewport_z_override
        @viewport_z_override = value.to_i
        yield
      ensure
        @viewport_z_override = previous
      end
...
class ReloadedMartBuyAdapter
  ...
  def currency(entry = nil)
    return :money unless entry
    result = price(entry)
    (result && result.currency ? result.currency : entry.currency || :money).to_sym
  rescue
    :money
  end

  def balance(entry = nil)
    ReloadedMart::Inventory.currency_balance(currency(entry)).to_i
  rescue
    0
  end
  ...
  def money_str(entry = nil)
    ReloadedMart.format_currency(balance(entry), currency(entry))
  end
  ...
  def max_quantity(entry)
    ...
    max = [max, 1].min if [:service, :unlock].include?(entry.kind)
    unit = buy_price(entry)
    max = [max, (balance(entry) / unit).floor].min if unit > 0
    ...
    highest_storable_quantity(entry, max)
  rescue Exception => e
    ReloadedMart.log_exception("Mart UI max quantity failed for #{entry&.id}", e)
    0
  end

  def highest_storable_quantity(entry, max)
    handler = ReloadedMart.entry_handler(entry.kind) || ReloadedMart::EntryHandler.new(entry.kind)
    low = 1
    high = max.to_i
    best = 0
  ...
  def mystery_box_max_quantity(entry, max)
    possible_grants = Array(entry.grants).map do |grant|
      payload = mystery_outcome_payload(grant)
      Reloaded::Rewards.normalize(payload) if defined?(Reloaded::Rewards)
    end.compact
    return max if possible_grants.empty?
    possible_grants.any? do |reward|
      Reloaded::Rewards.validate(reward, :source => :reloaded_mart).ok?
    end ? max : 0
  rescue Exception => e
  ...
  def mystery_outcome_payload(outcome)
    ...
    {
      :type => :group,
      :name => outcome["name"] || outcome[:name] || outcome["label"] || outcome[:label] || "Mystery Reward",
  ...
  def bundle_preview_lines(entry)
    ...
    lines = [mystery_box?(entry) ? "Possible contents:" : "Will receive:"]
    Array(entry.grants).each do |grant|
      reward = Reloaded::Rewards.normalize(mystery_box?(entry) ? mystery_outcome_payload(grant) : grant) if defined?(Reloaded::Rewards)
      next unless reward
      if reward[:type] == :item
        data = GameData::Item.try_get(reward[:item_id]) rescue nil
        owned = data ? ReloadedMart::Inventory.quantity(data.id) : 0
        label = data ? item_display_name(data) : Reloaded::Rewards.label(reward)
        lines << "#{label} x#{reward[:quantity]}   Owned: #{owned}"
      else
        lines << Reloaded::Rewards.label(reward)
      end
    end
    lines
  end
...
class ReloadedMartBuyScene
  ...
  def pbStartBuyScene(adapter)
    ...
    setup_entry_list_state(@entry_index)
    snap_quantity
    draw_all
    Graphics.transition(8)
    start_catalog_refresh_after_open
  ...
  def pbChooseBuyNumber(entry, maximum)
    ...
    currency = @adapter.respond_to?(:currency) ? @adapter.currency(entry) : :money
    result = Reloaded::NumberPicker.quantity(
      _INTL("Choose Quantity"),
      :label => @adapter.display_name(entry),
      :min => 1,
      :max => maximum,
      :initial => 1,
      :step => 1,
      ...
      :currency_formatter => proc { |value| ReloadedMart.format_currency(value, currency) },
      :preview_color => proc { |total, _quantity| total.to_i <= 0 ? GREEN : RED }
    )
  ...
  def pbConfirmPurchase(entry, quantity, total)
    ...
    Reloaded::NumberPicker.confirm(
      ...
      :initial => quantity,
      :value_prefix => "x",
      :unit_price => @adapter.buy_price(entry),
      :show_unit_price => true,
      :preview => proc { |_value| total },
      :currency_formatter => proc { |value| ReloadedMart.format_currency(value, currency) },
      :preview_color => proc { |amount, _value| amount.to_i <= 0 ? GREEN : RED },
      :default => true
    )
  ensure
    entry_list_dialog_closed!
  ...
  def mystery_box_reveal_rows(result)
    ...
    raw.each_with_index.map do |grant, index|
      ...
      if reward && reward[:type] == :item
        ...
        item_id = grant[:item_id] || grant["item_id"] || grant[:item] || grant["item"]
      end
  ...
  def setup_sprites
    ...
    @viewport.z = ReloadedMart::UI.viewport_z
  ...
  def rebuild_pockets
    selected_id = entry_list_row_id(selected_entry)
    ...
    setup_entry_list_state(@entry_index, selected_id) if @list_state
  end
  ...
  def ensure_visible
    ...
    sync_entry_list_state
  end

  def list_state_owner
    :buy
  end

  def entry_list_row_id(entry)
    return nil unless entry
    value = entry.respond_to?(:id) ? entry.id : entry
    value.to_s
  rescue
    entry.to_s
  end

  def entry_list_memory_key
    pocket = @pockets[@pocket_index] rescue nil
    pocket_id = pocket ? (pocket[:id] || pocket[:name]) : @pocket_index
    [:reloaded_mart, list_state_owner, pocket_id.to_s]
  end

  def setup_entry_list_state(initial_index = @entry_index, initial_id = nil)
    key = entry_list_memory_key
    if @list_state && @entry_list_key == key
      @list_state.replace_rows(current_entries, :preserve => :id)
      @list_state.select_id(initial_id) unless initial_id.nil?
    else
      @list_state = Reloaded::ListState.new(
        :rows => current_entries,
        :visible_rows => proc { rows_per_page },
        :row_id => proc { |entry, _index| entry_list_row_id(entry) },
  ...
  def sync_entry_list_state
    return unless @list_state
    @entry_index = @list_state.index || 0
    @scroll = @list_state.scroll
  end

  def entry_list_dialog_closed!
  ...
  def handle_entry_input
    ...
    event = @list_state.update_input(:mouse => false, :accept => false, :back => false)
    if event.moved?
      pbPlayCursorSE rescue nil
      sync_entry_list_state
      @bundle_scroll = 0
  ...
  def preserve_entry_after_rebuild(entry)
    ...
    @entry_index = @entry_index.clamp(0, [current_entries.length - 1, 0].max)
    setup_entry_list_state(@entry_index, entry_list_row_id(entry))
    @info_scroll = 0
    @bundle_scroll = 0
    clear_info_detail_cache
    ensure_visible
  ...
  def handle_mouse
    ...
    if my.between?(LIST_Y, LIST_Y + LIST_H - 1)
      event = @list_state.update_input(
        :commands => false,
        :mouse_index => proc { |_x, y| @list_state.scroll + ((y - LIST_Y) / ROW_H).floor }
      )
      if event.moved?
        pbPlayCursorSE rescue nil
        sync_entry_list_state
      ...
      show_hint_popup if controls_mouse_at?(mx, my)
    end
  end
  ...
  def scroll_entry_list(delta)
    return if current_entries.empty? || !@list_state
    amount = delta.to_i > 0 ? -1 : 1
    event = @list_state.move(amount, :mouse, false)
  ...
  def scroll_info_panel(delta)
    entry = selected_entry
    return unless entry
    if entry.respond_to?(:bundle_like?) && entry.bundle_like?
  ...
  def mouse_pos
    ...
    Reloaded::MouseInput.active_position
  rescue
    nil
  end
  ...
  def draw_pocket_nav
    ...
    shadow_text(bitmap, PAD, -1, 102, POCKET_H, title, BLUE)
    shadow_text(bitmap, 102, -1, SW - 204, POCKET_H, "#{pocket[:name]}  (#{@pocket_index + 1}/#{@pockets.length})", WHITE, 1)
    selected = selected_entry
    selected_currency = @adapter.respond_to?(:currency) ? @adapter.currency(selected) : :money
    displayed_balance = selected_currency == :money ? @money_display : @adapter.balance(selected)
    shadow_text(bitmap, 0, -1, SW - PAD, POCKET_H, ReloadedMart.format_currency(displayed_balance, selected_currency), GREEN, 2)
  end
  ...
  def draw_entry_row(bitmap, entry, y, selected)
    ...
    price_badges = !locked && !maxed ? price_side_badges(bitmap, entry, state) : []
    ...
    unless price_badges.empty?
      ...
      change_badge = price_badges.first
      price_color = change_badge ? change_badge[:color] : price_color
    end
  ...
  def price_side_badges(bitmap, entry, state = nil)
    badges = []
  ...
  def draw_info
    ...
    unless entry
      ...
      no_shadow_text(bitmap, PAD, INFO_H / 2 - 8, SW - PAD * 2, 20, "Nothing to buy here.", DIM)
      return
    end
    icon_x = SW - PAD - 96
    ...
    pbSetSmallFont(bitmap)
    no_shadow_text(bitmap, PAD, 4, icon_x - PAD * 2 - 4, 20, @adapter.display_name(entry), WHITE)
    time_text = @adapter.limited_time_text(entry)
    if time_text && !time_text.to_s.empty?
      bitmap.font.size = 14
      no_shadow_text(bitmap, PAD + 112, 6, icon_x - PAD * 2 - 166, 14, time_text, GOLD, 1)
      pbSetSmallFont(bitmap)
    end
    meta = info_meta(entry)
    no_shadow_text(bitmap, PAD, 4, icon_x - PAD - 10, 20, meta, meta_color(entry), 2)
  ...
  def draw_info_body(bitmap, entry, icon_x)
    ...
    if entry.bundle_like?
      ...
      lines[@info_scroll, 4].to_a.each do |line|
        ...
        no_shadow_text(bitmap, x, y, width, 18, line, color)
        y += 18
      end
    end
    details = []
    details << "Restock: #{@adapter.restock_text(entry)}" if @adapter.restock_text(entry)
    no_shadow_text(bitmap, x, INFO_H - 20, width, 16, details.compact.join("  "), GOLD) unless details.compact.empty?
  end
  ...
  def draw_bundle_preview(bitmap, entry, x, y, width)
    ...
    heading = @adapter.mystery_box?(entry) ? "Possible contents:" : "Will receive:"
    no_shadow_text(bitmap, x, y, width, 15, heading, WHITE)
    stock = @adapter.stock_remaining(entry)
    no_shadow_text(bitmap, 0, y, x + width, 15, "Stock: #{stock}", DIM, 2) unless stock.nil?
    y += 15
    item_w = width - 118
    owned_x = x + width - 92
    grants[@bundle_scroll, visible_rows].to_a.each do |grant|
      label = grant[:quantity] > 1 ? "#{grant[:quantity]}x #{grant[:name]}" : grant[:name]
      color = grant[:rarity] ? mystery_box_rarity_color(grant[:rarity]) : GRAY
      no_shadow_text(bitmap, x + 6, y, item_w, 14, trim_text(bitmap, label, item_w), color)
      no_shadow_text(bitmap, owned_x, y, 92, 14, "Owned: #{grant[:owned]}", DIM, 2) unless grant[:owned].nil?
      y += 14
    end
  ...
  def build_normalized_bundle_grants(entry)
    Array(entry.grants).map do |grant|
      raw = grant.is_a?(Hash) ? grant : {}
      grants = raw["grants"] || raw[:grants]
      payload = if @adapter.mystery_box?(entry) && grants.is_a?(Array)
                  {
                    :type => :group,
                    :name => raw["name"] || raw[:name] || raw["label"] || raw[:label] || "Mystery Reward",
                    :grants => grants
                  }
                else
                  grant
                end
      reward = Reloaded::Rewards.normalize(payload) if defined?(Reloaded::Rewards)
      next nil unless reward
      if reward[:type] == :item
        data = GameData::Item.try_get(reward[:item_id]) rescue nil
        next nil unless data
        { :name => item_display_name(data), :quantity => reward[:quantity], :owned => ReloadedMart::Inventory.quantity(data.id), :rarity => raw["rarity"] || raw[:rarity] }
      else
        { :name => Reloaded::Rewards.label(reward), :quantity => reward[:quantity], :owned => nil, :rarity => raw["rarity"] || raw[:rarity] }
      end
    end.compact
  end
  ...
  def controls_mouse_at?(mx, my)
    ...
    Reloaded::HintText.controls_at?(@footer_sprite.bitmap, mx, my - (SH - FOOTER_H), PAD, 2, SW - PAD * 2, :size => 16, :height => FOOTER_H)
  rescue
    false
  end
  ...
  def show_hint_popup
    ...
    Reloaded::HintText.open_popup("RLD Mart Hints", hint_entries, :statuses => hint_statuses) if defined?(Reloaded::HintText)
    draw_all
  rescue
    draw_all rescue nil
  ensure
  ...
  def draw_cursor(bitmap, x, y, width, height)
    fill_base = respond_to?(:reloaded_cursor_fill) ? reloaded_cursor_fill : Color.new(100, 160, 220, 160)
    border_base = respond_to?(:reloaded_cursor_border) ? reloaded_cursor_border : Color.new(60, 120, 180, 220)
    ...
    fill = Color.new(fill_base.red, fill_base.green, fill_base.blue, fill_alpha)
    border = Color.new(border_base.red, border_base.green, border_base.blue, border_alpha)
    if respond_to?(:reloaded_draw_rounded_rect)
    ...
    bitmap.fill_rect(x, y, width, height, Color.new(100, 160, 220, 170)) rescue nil
  end

  def poll_catalog_refresh
    ...
    ReloadedMart.log_exception("Reloaded Mart catalog refresh polling failed", e) if defined?(ReloadedMart)
    false
  end
  ...
  def draw_modal_choices(bitmap, choices, selected, x, y, width, line_h)
    choices.each_with_index do |choice, i|
      ...
      plain_text(bitmap, x + 8, row_y - 6, width - 16, line_h, choice, color)
    end
  end
  ...
  def no_shadow_text(bitmap, x, y, width, height, text, color, align = 0)
    pbDrawShadowText(bitmap, x, y, width, height, text.to_s, color, Color.new(0, 0, 0, 0), align)
  rescue
  end
  ...
  def show_message(message)
    if defined?(Reloaded) && Reloaded.respond_to?(:toast_ok)
      Reloaded.toast_ok(message.to_s)
      draw_content rescue nil
      return true
    elsif defined?(Reloaded) && Reloaded.respond_to?(:message)
      Reloaded.message(message.to_s)
      draw_content rescue nil
      return true
    end
    ...
    y = pad
    lines.each do |line|
      plain_text(bitmap, pad, y - 6, box_w - pad * 2, line_h, line, WHITE)
      y += line_h
    end
    bitmap.font.size = 16
    plain_text(bitmap, pad, box_h - pad - 22, box_w - pad * 2, 16, "Confirm (C)", GRAY, 2)
    loop do
      Graphics.update
      Input.update
...
class ReloadedMartSellScene < ReloadedMartBuyScene
  ...
  def pbChooseSellNumber(item, maximum)
    ...
    result = Reloaded::NumberPicker.quantity(
      _INTL("Choose Quantity"),
      :label => @adapter.display_name(item),
      :min => 1,
      :max => maximum,
      :initial => 1,
      :step => 1,
      ...
      :unit_price => unit,
      :show_unit_price => true,
      :currency_formatter => proc { |value| ReloadedMart.format_currency(value) },
      :preview_color => GREEN
    )
    result.nil? ? 0 : result.to_i
  ensure
  ...
  def pbConfirmSale(item, quantity, total)
    ...
    Reloaded::NumberPicker.confirm(
      ...
      :initial => quantity,
      :value_prefix => "x",
      :unit_price => @adapter.sell_price(item),
      :show_unit_price => true,
      :preview => proc { |_value| total },
      :currency_formatter => proc { |value| ReloadedMart.format_currency(value) },
      :preview_color => GREEN,
  ...
  def draw_info
    ...
    unless item
      @icon_sprite.item = nil if @icon_sprite
      @special_icon_sprite.visible = false if @special_icon_sprite
      no_shadow_text(bitmap, PAD, INFO_H / 2 - 8, SW - PAD * 2, 20, "Nothing to sell here.", DIM)
      return
    end
    icon_x = SW - PAD - 96
    draw_icon_box(bitmap, icon_x, 4, 96, 96)
    ...
    pbSetSmallFont(bitmap)
    no_shadow_text(bitmap, PAD, 4, icon_x - PAD * 2 - 4, 20, @adapter.display_name(item), WHITE)
    no_shadow_text(bitmap, PAD, 4, icon_x - PAD - 10, 20, "x#{@qty_display.round}", WHITE, 2)
    bitmap.fill_rect(PAD + 2, 27, icon_x - PAD - 4, 1, SEP)
    y = 34
    width = icon_x - PAD * 2 - 4
    bitmap.font.size = 16
  ...
  def hint_entries
    ...
    mode = @sell_all_mode ? "On" : "Off"
    [
      Reloaded::HintText.confirm("Sell"),
      Reloaded::HintText.back,
      Reloaded::HintText.other("Page", :page),
      Reloaded::HintText.other("Sort: #{sort}", :sort),
      Reloaded::HintText.other("Sell-All: #{mode}", :quick)
    ]
  rescue
    []
  end
  ...
  def hint_statuses
    statuses = [Reloaded::HintText.status("Sort: #{sell_sort_label}", BLUE)]
    statuses << Reloaded::HintText.status("Sell-All Mode", GREEN) if @sell_all_mode
    statuses
  rescue
    []
  end
  ...
  def sell_sort_label
    case ReloadedMartSellAdapter::SORT_MODES[@sort_index]
    when :price_high then "Price High"
    when :price_low then "Price Low"
    else "Name"
    end
