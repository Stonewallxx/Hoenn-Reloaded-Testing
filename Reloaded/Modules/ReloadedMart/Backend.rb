...
module ReloadedMart
  ...
  class CatalogEntry
    attr_accessor :id, :kind, :name, :category_id, :category_ids, :category_name, :tags,
                  ...
                  :entry_version, :online_policy, :source_type,
                  :raw
    ...
    def initialize(attrs = {})
      ...
      @entry_version = [attrs[:entry_version].to_i, 1].max
      @online_policy = (attrs[:online_policy] || :fresh_required).to_sym
      @source_type   = (attrs[:source_type] || :curated).to_sym
      @raw           = attrs[:raw]
    end
  ...
  class BundleEntryHandler < EntryHandler
    ...
    def grants_for(entry, quantity, _context = {})
      ...
      return [] unless defined?(Reloaded::Rewards)
      Array(entry.grants).flat_map { |grant| Reloaded::Rewards.expand(grant, quantity) }
    rescue Exception => e
      ReloadedMart.log_exception("Bundle reward expansion failed", e)
      []
    end
    ...
    def validate(line, _context = {})
      ...
      grants.each do |grant|
        reward = Reloaded::Rewards.normalize(grant) if defined?(Reloaded::Rewards)
        return TransactionResult.new(false, :invalid_bundle_grant, "This bundle is unavailable.") unless reward
        return TransactionResult.new(false, :invalid_bundle_grant, "This bundle is unavailable.") if reward[:quantity].to_i <= 0
      end
      TransactionResult.new(true, :ok, "")
    rescue Exception => e
    ...
    def mystery_grants_for(entry, quantity)
      ...
      count.times do |index|
        ...
        reward = Reloaded::Rewards.normalize(mystery_outcome_payload(grant)) if defined?(Reloaded::Rewards)
        next unless reward
        reward[:rarity] = mystery_grant_rarity(grant)
        ReloadedMart.log_info(
          "Mystery reward roll entry=#{entry&.id} roll_index=#{index + 1} #{Reloaded::Rewards.description(reward)} rarity=#{reward[:rarity] || "none"} weight=#{mystery_grant_weight(grant)} mode=#{roll_details[:mode]} roll=#{roll_details[:roll]}/#{roll_details[:total]} threshold=#{roll_details[:threshold] || "-"}"
        )
        grants << reward
      end
      grants
    ...
    def mystery_outcome_payload(outcome)
      return outcome unless outcome.is_a?(Hash)
      grants = outcome["grants"] || outcome[:grants]
      return outcome unless grants.is_a?(Array)
      {
        :type => :group,
        :name => outcome["name"] || outcome[:name] || outcome["label"] || outcome[:label] || "Mystery Box Reward",
        :rarity => outcome["rarity"] || outcome[:rarity],
        :grants => grants
      }
    end
    ...
    def mystery_grant_weight(grant)
      ...
      value = grant["chance"] || grant[:chance] || grant["weight"] || grant[:weight]
      [value.to_i, 0].max
    end
  ...
  class << self
    ...
    def register_patch_points
      ...
      Reloaded::Patches.register(
        ...
        :type => :wrap,
        :file => "Reloaded/Modules/ReloadedMart/UI.rb",
        :owner => :reloaded,
        :priority => 100,
    ...
    def open
      ...
      if ui_ready?
        ...
        popup_message(_INTL("The Reloaded Mart is being prepared."))
        log_warning("Reloaded Mart UI is unavailable")
      end
    rescue Exception => e
      raise if e.is_a?(SystemExit)
      log_exception("Reloaded Mart open failed", e)
      popup_message(_INTL("The Reloaded Mart is unavailable right now."))
    end
    ...
    def open_options
      ...
      popup_message(_INTL("Reloaded Mart options are unavailable right now."))
    end
    ...
    def format_currency(amount, currency = :money)
      key = currency.to_sym
      value = amount.to_i.to_s.reverse.gsub(/(\d{3})(?=\d)/, '\\1,').reverse
      symbol = currency_symbol(key)
      return "#{symbol}#{value}" if key == :money
      return "#{value} #{symbol}" unless symbol.to_s.empty?
      return Reloaded::Rewards.format_currency(key, amount) if defined?(Reloaded::Rewards) && Reloaded::Rewards.respond_to?(:format_currency)
      value
    end
  ...
  module Source
    ...
    class << self
      ...
      def clear_cache
        ...
        ReloadedMart.log_exception("Reloaded Mart cache clear failed", e)
        false
      end
      ...
      def refresh_online(blocking: true)
        ...
        ensure_remote_source
        remote = Reloaded::RemoteData.fetch(REMOTE_SOURCE_ID, :force => true)
        unless remote.ok?
          ReloadedMart.log_warning("Mart catalog fetch failed reason=#{remote.error_code || :unavailable}")
          return load_cached_or_fail(remote.error_code || :remote_unavailable)
        end
        apply_remote_result(remote)
        current
      rescue Exception => e
      ...
      def apply_remote_result(remote)
        ...
        ReloadedMart.log_exception("Mart catalog apply failed", e)
        load_cached_or_fail(:exception)
        false
      end
  ...
  module Economy
    class << self
      ...
      def daily_featured_config
        return DEFAULT_DAILY_FEATURED unless Source.online_fresh?
  ...
  module Inventory
    class << self
      ...
      def currency_balance(currency = :money)
        key = currency.to_sym
        return money if key == :money && (!defined?(Reloaded::Rewards) || !Reloaded::Rewards.respond_to?(:currency_balance))
        value = Reloaded::Rewards.currency_balance(key) if defined?(Reloaded::Rewards) && Reloaded::Rewards.respond_to?(:currency_balance)
        value.nil? && key == :money ? money : value
      rescue
        key == :money ? money : nil
      end

      def currency_name(currency = :money)
        return Reloaded::Rewards.currency_name(currency) if defined?(Reloaded::Rewards) && Reloaded::Rewards.respond_to?(:currency_name)
        currency.to_s.split("_").map { |part| part.capitalize }.join(" ")
      rescue
        currency.to_s
      end

      def can_afford?(amount, currency = :money)
        balance = currency_balance(currency)
        !balance.nil? && balance >= amount.to_i
      end

      def charge(amount, currency = :money)
        price = amount.to_i
        key = currency.to_sym
        unless can_afford?(price, key)
          return TransactionResult.new(false, :not_enough_currency, _INTL("You don't have enough {1}.", currency_name(key)), :currency => key, :needed => price, :available => currency_balance(key).to_i)
        end
        if defined?(Reloaded::Rewards) && Reloaded::Rewards.respond_to?(:spend_currency)
          result = Reloaded::Rewards.spend_currency(key, price)
          return TransactionResult.new(false, result.code, result.message, result.details) unless result.ok?
          return TransactionResult.new(true, :ok, "", result.details)
        end
        return TransactionResult.new(false, :currency_write_failed, "The transaction could not be completed.") unless key == :money && set_money(money - price)
        TransactionResult.new(true, :ok, "", :currency => key, :amount => price)
      end

      def refund(amount, currency = :money)
        key = currency.to_sym
        if defined?(Reloaded::Rewards) && Reloaded::Rewards.respond_to?(:refund_currency)
          return Reloaded::Rewards.refund_currency(key, amount.to_i).ok?
        end
        key == :money && set_money(money + amount.to_i)
      end
      ...
      def normalize_grants(grants)
        return { :ok => false, :code => :rewards_unavailable, :message => "The reward system is unavailable." } unless defined?(Reloaded::Rewards)
        rewards = Reloaded::Rewards.normalize_all(grants)
        return { :ok => false, :code => :invalid_reward, :message => "One of the rewards is unavailable." } if rewards.length != Array(grants).length
        items = rewards.select { |reward| reward[:type] == :item }
        special = rewards.reject { |reward| reward[:type] == :item }
        { :ok => true, :grants => items, :special_grants => special, :rewards => rewards }
      rescue Exception => e
        ReloadedMart.log_exception("Failed to normalize Mart rewards", e)
      ...
      def can_store_grants?(grants)
        ...
        result = Reloaded::Rewards.validate_all(normalized[:rewards], :source => :reloaded_mart, :ignore_iv_boundaries => true)
        return TransactionResult.new(false, result.code, result.message, result.details) unless result.ok?
        TransactionResult.new(true, :ok, "", :grants => normalized[:grants], :special_grants => normalized[:special_grants], :rewards => normalized[:rewards])
      rescue Exception => e
        ReloadedMart.log_exception("Mart reward preflight failed", e)
        TransactionResult.new(false, :reward_preflight_failed, "The rewards could not be granted.")
      end

      def apply_grants(grants)
        normalized = normalize_grants(grants)
        return TransactionResult.new(false, normalized[:code], normalized[:message], :item_id => normalized[:item_id]) unless normalized[:ok]
        result = Reloaded::Rewards.grant_all(
          normalized[:rewards],
          :source => :reloaded_mart,
          :ignore_iv_boundaries => true,
          :notify => false,
          :defer_finalize => true
        )
        return TransactionResult.new(false, result.code, result.message, result.details) unless result.ok?
        TransactionResult.new(true, :ok, "", :applied => result.details[:applied], :receipts => result.details[:receipts])
      rescue Exception => e
        ReloadedMart.log_exception("Mart reward application failed", e)
        TransactionResult.new(false, :grant_exception, "The transaction could not be completed.")
      end

      def rollback_grants(grants)
        rows = Array(grants)
        if defined?(Reloaded::Rewards::Receipt) && rows.all? { |row| row.is_a?(Reloaded::Rewards::Receipt) }
          return Reloaded::Rewards.rollback_all(rows, :source => :reloaded_mart_rollback)
        end
        return true unless defined?($PokemonBag) && $PokemonBag
        rows.reverse_each do |grant|
          data = resolve_item(grant[:item_id] || grant["item_id"] || grant[:id] || grant["id"])
      ...
      def register_tm_vault_grants(grants)
        # Item reward handlers register machine moves centrally.
        true
      rescue Exception => e
    ...
    class << self
      ...
      def validate_entry_fields(entry, report)
        ...
        if mystery_box_entry?(entry)
          random = Array(entry.grants).find do |grant|
      ...
      def catalog_grant_payload(entry, grant)
        return grant unless mystery_box_entry?(entry) && grant.is_a?(Hash)
        grants = grant["grants"] || grant[:grants]
        return grant unless grants.is_a?(Array)
        {
          :type => :group,
          :name => grant["name"] || grant[:name] || grant["label"] || grant[:label] || "Mystery Box Reward",
          :rarity => grant["rarity"] || grant[:rarity],
          :grants => grants
        }
      end
    ...
    class << self
      ...
      def bundle_entry_hash(bundle)
        mystery = truthy?(bundle["mystery_box"] || bundle["mystery"] || bundle["hidden_contents"])
        {
          ...
          "display" => {
            "description" => mystery ? (bundle["description"] || "Contains one possible reward outcome.") : bundle["description"],
            "mystery_box" => mystery
          },
  ...
  module Ledger
    ...
    class << self
      def prepare(cart, context = {})
        sequence = ReloadedMart.state(:transaction_sequence, 0).to_i + 1
        ReloadedMart.set_state(:transaction_sequence, sequence)
        id = transaction_id(sequence)
        record = {
          "id" => id,
          "status" => "prepared",
          "catalog_version" => catalog_version,
          "created_at" => Time.now.to_i,
          "currency" => Transactions.cart_currency(cart).to_s,
          "total" => cart.total_price.to_i,
          "source" => (cart.source rescue context[:source] || :reloaded_mart).to_s,
          "requires_save" => randomized?(cart),
          "lines" => serialize_lines(cart)
        }
        append(record)
        unless !record["requires_save"] || force_save
          update(id, "status" => "save_failed", "error" => "prepare_save_failed")
          return TransactionResult.new(false, :save_failed, "The game could not be saved, so this purchase was cancelled.", :transaction_id => id)
        end
        TransactionResult.new(true, :ok, "", :transaction_id => id, :record => record)
      rescue Exception => e
        ReloadedMart.log_exception("Mart transaction preparation failed", e)
        TransactionResult.new(false, :ledger_failed, "The purchase could not be prepared safely.")
      end
      ...
      def complete(id, result)
        ...
        update(id,
          ...
          "revealed" => serializable(Array(details[:revealed] || details["revealed"])),
          "distribution_ids" => distribution_ids(details)
        )
        record = find(id)
        saved = !record || !record["requires_save"] || force_save
        ReloadedMart.log_warning("Mart transaction completion save failed id=#{id}") unless saved
        saved
      rescue Exception => e
        ReloadedMart.log_exception("Mart transaction completion recording failed", e)
        false
      end

      def fail(id, stage, result = nil)
        return false if id.to_s.empty?
        update(id,
          "status" => "failed",
          "failed_at" => Time.now.to_i,
          "stage" => stage.to_s,
          "error" => (result.respond_to?(:code) ? result.code : result).to_s
        )
        true
      rescue Exception => e
        ReloadedMart.log_exception("Mart transaction failure recording failed", e)
        false
      end
      ...
      def recover_incomplete!
        ...
        history.each do |record|
          next unless record.is_a?(Hash) && record["status"].to_s == "prepared"
          record["status"] = "abandoned"
          record["recovered_at"] = Time.now.to_i
          changed = true
        end
        write(history) if changed
        changed
      rescue Exception => e
        ReloadedMart.log_exception("Mart transaction recovery failed", e)
        false
      end

      def history
        value = ReloadedMart.state(:transactions, [])
      ...
      def pending_deliveries
        value = ReloadedMart.state(:pending_deliveries, [])
        value.is_a?(Array) ? value : []
      rescue
        []
      end
      ...
      def append(record)
        rows = history
        rows << record
        rows.shift while rows.length > HISTORY_LIMIT
        write(rows)
      end

      def update(id, values)
        rows = history
        row = rows.reverse.find { |entry| entry.is_a?(Hash) && entry["id"].to_s == id.to_s }
      ...
      def write(rows)
        ReloadedMart.set_state(:transactions, rows)
      end

      def find(id)
        history.reverse.find { |row| row.is_a?(Hash) && row["id"].to_s == id.to_s }
      end

      def transaction_id(sequence)
        version = catalog_version.to_s.gsub(/[^A-Za-z0-9_.-]+/, "-")
        "#{version}:#{sequence.to_i}"
      end
      ...
      def randomized?(cart)
        Array(cart&.lines).any? do |line|
          display = line.entry.display.is_a?(Hash) ? line.entry.display : {}
          raw = line.entry.raw.is_a?(Hash) ? line.entry.raw : {}
          Rules.truthy?(display["mystery_box"] || display[:mystery_box] || raw["mystery_box"] || raw[:mystery_box])
        end
      rescue
        false
      ...
      def serialize_lines(cart)
        Array(cart&.lines).map do |line|
          {
            "entry_id" => line.entry_id.to_s,
            "entry_version" => (line.entry.entry_version rescue 1).to_i,
            "kind" => line.entry_kind.to_s,
            "quantity" => line.quantity.to_i,
            "unit_price" => (line.price_result&.final || 0).to_i,
            "grants" => serializable(line.grants)
          }
        end
      end

      def distribution_ids(details)
        collect_distribution_ids(details[:receipts] || details["receipts"]).uniq
      rescue
        []
      end

      def collect_distribution_ids(receipts)
        Array(receipts).each_with_object([]) do |receipt, ids|
          next unless receipt
          reward = receipt.respond_to?(:reward) ? receipt.reward : nil
          if reward.is_a?(Hash) && reward[:type].to_s == "pokemon"
            id = reward[:distribution_id].to_s
            ids << id unless id.empty?
          end
          data = receipt.respond_to?(:data) ? receipt.data : nil
          child_receipts = data[:receipts] || data["receipts"] if data.is_a?(Hash)
          ids.concat(collect_distribution_ids(child_receipts)) if child_receipts
        end
      end
      ...
      def force_save
        return false unless defined?(Game) && Game.respond_to?(:save)
        !!Game.save(safe: true)
      rescue Exception => e
        ReloadedMart.log_exception("Mart transaction save failed", e)
        false
      end
    end
  end
  ...
  module Transactions
    class << self
      ...
      def validate_cart(cart, context = {})
        ...
        return TransactionResult.new(false, :mixed_currency, "A purchase cannot mix currencies.") if currency == :mixed
        return TransactionResult.new(false, :unsupported_currency, "This currency is unavailable.") if Inventory.currency_balance(currency).nil?
        ...
        unless Inventory.can_afford?(cart.total_price, currency)
          return TransactionResult.new(false, :not_enough_currency, _INTL("You don't have enough {1}.", Inventory.currency_name(currency)), :currency => currency, :needed => cart.total_price, :available => Inventory.currency_balance(currency).to_i)
        end
      ...
      def complete_cart(cart, context = {})
        ...
        prepared = Ledger.prepare(cart, context)
        unless prepared.ok?
          log_purchase_failure(cart, prepared, :prepare, context)
          ReloadedMart.emit(EVENT_PURCHASE_FAILED, event_context(cart, prepared, context))
          return prepared
        end
        transaction_id = prepared.details[:transaction_id]
        amount = cart.total_price
        currency = cart_currency(cart)
        charge_amount = immediate_charge_amount(cart, context)
        charge = Inventory.charge(charge_amount, currency)
        unless charge.ok?
          Ledger.fail(transaction_id, :charge, charge)
        ...
        unless grants.ok?
          Inventory.refund(charge_amount, currency)
          Ledger.fail(transaction_id, :grant, grants)
          log_purchase_failure(cart, grants, :grant, context)
          ReloadedMart.emit(EVENT_PURCHASE_FAILED, event_context(cart, grants, context))
          return grants
        end
        grant_receipts = Array(grants.details[:receipts])
        apply = apply_handlers(cart, context)
        unless apply.ok?
          Inventory.rollback_grants(grants.details[:receipts] || grants.details[:applied])
          rollback_handler_side_effects(context)
          Inventory.refund(charge_amount, currency)
          Ledger.fail(transaction_id, :handler, apply)
          log_purchase_failure(cart, apply, :handler, context)
          ReloadedMart.emit(EVENT_PURCHASE_FAILED, event_context(cart, apply, context))
          return apply
        end
        Reloaded::Rewards.finalize_all(
          grants.details[:receipts],
          context.merge(:source => :reloaded_mart, :notify => false)
        )
        ...
        result = TransactionResult.new(
          ...
          :ok,
          "Purchase complete.",
          :applied => grants.details[:applied],
          :receipts => grants.details[:receipts],
          :revealed => Reloaded::Rewards.revealed_rewards(grants.details[:receipts]),
          :transaction_id => transaction_id
        )
        Ledger.complete(transaction_id, result)
        ...
        unless committed
          Inventory.rollback_grants(grant_receipts) unless grant_receipts.empty?
          rollback_handler_side_effects(context)
          Inventory.refund(charge_amount, currency) if charged && charge_amount.to_i > 0
        end
        Ledger.fail(transaction_id, :exception, :exception) if transaction_id
      ...
      def rollback_handler_side_effects(context = {})
        ...
        Array(context[:deferred_charges]).each do |charge|
          if charge.is_a?(Hash)
            ...
            Inventory.refund(charge.to_i, :money)
          end
        end
      ...
      def grant_log_summary(grants)
        rows = Array(grants).map do |grant|
          if defined?(Reloaded::Rewards) && grant.is_a?(Hash) && grant[:type]
            next Reloaded::Rewards.description(grant)
          end
