...
module TMVault
  ...
  class Scene
    ...
    def clamp_scroll
      max = [(@list.length - rows_per_page), 0].max
      @scroll = @scroll.clamp(0, max)
      @sel    = @sel.clamp(0, [@list.length - 1, 0].max)
    end
      ...
      if Input.repeat?(Input::UP)
        ...
        unless @list.empty?
          if @sel < @list.length - 1
            @sel += 1
            @scroll = @sel - rpp + 1 if @sel >= @scroll + rpp
          else
            @sel = 0
            @scroll = 0
          end
