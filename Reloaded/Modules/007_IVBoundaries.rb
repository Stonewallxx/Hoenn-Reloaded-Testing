...
module ReloadedIVBoundaries
  ...
  class << self
    ...
    def install_mart_patch
      ...
      install_mart_bundle_patch
      install_mart_inventory_patch
      log_info("Installed IV Boundaries Reloaded Mart patch")
      true
    rescue Exception => e
    ...
    def install_mart_bundle_patch
      ReloadedMart::BundleEntryHandler.class_eval do
        unless method_defined?(:reloaded_iv_boundaries_original_grants_for)
          alias_method :reloaded_iv_boundaries_original_grants_for, :grants_for
          def grants_for(entry, quantity, context = {})
            grants = reloaded_iv_boundaries_original_grants_for(entry, quantity, context)
            unless mystery_box?(entry)
              Array(entry&.grants).each do |grant|
                next unless defined?(ReloadedIVBoundaries) && ReloadedIVBoundaries.iv_reward?(grant)
                [quantity.to_i, 1].max.times do
                  grants << ReloadedIVBoundaries.mart_special_grant(grant)
                end
              end
            end
            grants
        ...
        unless method_defined?(:reloaded_iv_boundaries_original_mystery_grants_for)
          alias_method :reloaded_iv_boundaries_original_mystery_grants_for, :mystery_grants_for
          def mystery_grants_for(entry, quantity)
            ...
            count.times do |index|
              ...
              if defined?(ReloadedIVBoundaries) && ReloadedIVBoundaries.iv_reward?(grant)
                reward = ReloadedIVBoundaries.mart_special_grant(grant)
                reward[:rarity] = mystery_grant_rarity(grant)
                grants << reward
                ReloadedMart.log_info(
                  "Mystery reward roll entry=#{entry&.id} roll_index=#{index + 1} type=#{reward[:type]} scope=#{reward[:scope]} rarity=#{reward[:rarity] || "none"} weight=#{mystery_grant_weight(grant)} mode=#{roll_details[:mode]} roll=#{roll_details[:roll]}/#{roll_details[:total]} threshold=#{roll_details[:threshold] || "-"}"
                )
                next
              end
        ...
        unless method_defined?(:reloaded_iv_boundaries_original_validate)
          alias_method :reloaded_iv_boundaries_original_validate, :validate
          def validate(line, _context = {})
            grants = Array(line&.grants)
            return ReloadedMart::TransactionResult.new(false, :empty_bundle, "This bundle is unavailable.") if grants.empty?
            grants.each do |grant|
              next if defined?(ReloadedIVBoundaries) && ReloadedIVBoundaries.iv_reward?(grant)
              next if grant.is_a?(Hash) && [:iv_boundary_boost, :iv_boundary_force_next].include?(grant[:type])
              if pokevial_grant?(grant)
                quantity = pokevial_quantity(grant).to_i
                return ReloadedMart::TransactionResult.new(false, :invalid_bundle_grant, "This bundle is unavailable.") if quantity <= 0
                next
              end
              ...
              return ReloadedMart::TransactionResult.new(false, :invalid_bundle_grant, "This bundle is unavailable.") if item_id.nil? || item_id.to_s.empty? || quantity <= 0
              data = GameData::Item.try_get(item_id) rescue nil
              return ReloadedMart::TransactionResult.new(false, :missing_item, "One of the items is unavailable.", :item_id => item_id) unless data
            end
            ReloadedMart::TransactionResult.new(true, :ok, "")
          rescue Exception => e
            ReloadedMart.log_exception("Bundle grant validation failed", e)
            ReloadedMart::TransactionResult.new(false, :bundle_validation_failed, "This bundle is unavailable.")
          end
        end
      end
    end
    ...
    def install_mart_inventory_patch
      singleton = class << ReloadedMart::Inventory; self; end
      unless singleton.method_defined?(:reloaded_iv_boundaries_original_normalize_grants)
        singleton.class_eval do
          alias_method :reloaded_iv_boundaries_original_normalize_grants, :normalize_grants
          def normalize_grants(grants)
            iv_special = []
            filtered = []
            Array(grants).each do |grant|
              if defined?(ReloadedIVBoundaries) && ReloadedIVBoundaries.iv_reward?(grant)
                iv_special << ReloadedIVBoundaries.mart_special_grant(grant)
              elsif grant.is_a?(Hash) && [:iv_boundary_boost, :iv_boundary_force_next].include?(grant[:type])
                iv_special << grant
              else
                filtered << grant
              end
            end
            result = reloaded_iv_boundaries_original_normalize_grants(filtered)
            result[:special_grants] ||= []
            result[:special_grants].concat(iv_special)
            result
          end
        end
      end
      unless singleton.method_defined?(:reloaded_iv_boundaries_original_apply_grants)
        singleton.class_eval do
          alias_method :reloaded_iv_boundaries_original_apply_grants, :apply_grants
          def apply_grants(grants)
            result = reloaded_iv_boundaries_original_apply_grants(grants)
            return result unless result.respond_to?(:ok?) && result.ok?
            applied = Array(result.details[:applied])
            normalized = normalize_grants(grants)
            Array(normalized[:special_grants]).each do |grant|
              next unless [:iv_boundary_boost, :iv_boundary_force_next].include?(grant[:type])
              ok = ReloadedIVBoundaries.apply_mart_special_grant(grant) if defined?(ReloadedIVBoundaries)
              next unless ok
              applied << grant
            end
            result.details[:applied] = applied
            result
          end
        end
      end
