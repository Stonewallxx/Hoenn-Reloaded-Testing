...
#   - Provide a Reloaded-friendly registration API for future options.
#   - Store player-facing settings through PokemonSystem/Reloaded options.
#   - Register through the existing map-update event hook without base edits.
#   - Keep module logging and patch metadata visible for troubleshooting.
#
#======================================================
  ...
  module OverworldMenuFeature
    class << self
      def install
        ...
        Reloaded::Log.exception("Overworld Menu install failed", e, channel: :modules) if defined?(Reloaded::Log)
        false
      end
      ...
      def install_pokemon_system_settings
        ...
        PokemonSystem.class_eval do
          def hr_om_enabled
            @hr_om_enabled.nil? ? 1 : @hr_om_enabled.to_i
          end

          def hr_om_enabled=(value)
            @hr_om_enabled = value.to_i
          end

          def hr_om_party_view
            @hr_om_party_view.nil? ? false : !!@hr_om_party_view
          end

          def hr_om_party_view=(value)
            @hr_om_party_view = value == true || value.to_i == 1
          rescue
            @hr_om_party_view = !!value
          end
        end
      end
...
module OverworldMenu
  ...
  class << self
    ...
    def default_pages
      [{ "name" => "Main", "entries" => all_registered_keys.map { |key| key.to_s }, "disabled_entries" => [] }]
    end

    def pages
      raw = if defined?(Reloaded::SaveData)
              Reloaded::SaveData.get(SAVE_SYSTEM, :pages, nil, section: :systems)
            else
              @fallback_pages
            end
    ...
    def pages=(value)
      ...
      if defined?(Reloaded::SaveData)
        Reloaded::SaveData.set(SAVE_SYSTEM, :pages, normalized, section: :systems)
      else
    ...
    def page_index
      value = if defined?(Reloaded::SaveData)
                Reloaded::SaveData.get(SAVE_SYSTEM, :page_index, 0, section: :systems)
              else
                @fallback_page_index
              end
      value.to_i
    ...
    def page_index=(value)
      ...
      if defined?(Reloaded::SaveData)
        Reloaded::SaveData.set(SAVE_SYSTEM, :page_index, normalized, section: :systems)
      else
    ...
    def quick_items
      raw = if defined?(Reloaded::SaveData)
              Reloaded::SaveData.get(SAVE_SYSTEM, :quick_items, [], section: :systems)
            else
    ...
    def quick_items=(value)
      ...
      if defined?(Reloaded::SaveData)
        Reloaded::SaveData.set(SAVE_SYSTEM, :quick_items, normalized.map { |item| item ? item.to_s : nil }, section: :systems)
      else
    ...
    def enabled?
      ($PokemonSystem.hr_om_enabled rescue 1).to_i == 1
    end
    ...
    def register_patch_points
      ...
      Reloaded::Patches.register(
        :overworld_menu_map_update,
        :target => "Events.onMapUpdate",
        :type => :event_bridge,
        :file => __FILE__,
        :owner => :reloaded,
        :priority => 80,
        :reason => "Opens the Reloaded Overworld Menu from the overworld input loop.",
        :recommended_fix => "Check Modules/OverworldMenu.rb if the overworld quick menu fails to open.",
        :conflict_group => "overworld_menu",
        :allow_multiple => true
      )
    rescue Exception => e
      log_exception("Overworld Menu patch registration failed", e)
    end
    ...
    def normalize_pages(value)
      ...
      pages = Array(value).map.with_index do |page, index|
...
Reloaded::OverworldMenuFeature.install if defined?(Reloaded::OverworldMenuFeature)

class OverworldMenuScene
  SW = (defined?(Reloaded::ModManagerUI::UIHelpers) ? Reloaded::ModManagerUI::UIHelpers::SCREEN_W : 512) rescue 512
  SH = (defined?(Reloaded::ModManagerUI::UIHelpers) ? Reloaded::ModManagerUI::UIHelpers::SCREEN_H : 384) rescue 384
  ...
  def draw_panel_bg(bitmap, width, height, title)
    bitmap.fill_rect(0, 0, width, height, C_BG)
    bitmap.fill_rect(0, 0, width, HDR_H, C_HDR)
    ...
    bitmap.fill_rect(0, HDR_H - 1, width, 1, C_BORDER)
    pbSetSmallFont(bitmap)
    bitmap.font.size = 15
    pbDrawShadowText(bitmap, PAD, 4, width - PAD * 2, HDR_H - 6, title, C_WHITE, C_SHADOW, 1)
  end
  ...
  def build_icon_sprites
    ...
    party.each_with_index do |pkmn, i|
      ...
      col = i % SLOT_COLS
      row = i / SLOT_COLS
      sprite = PokemonIconSprite.new(pkmn, @vp)
...
class OverworldMenuScreen
  ...
  def open
    party_view = ($PokemonSystem.hr_om_party_view rescue false)
  ...
  def show_features_menu
    party_now = ($PokemonSystem.hr_om_party_view rescue false)
    result = @scene.show_features_menu(party_now)
    if result && result.has_key?(:party_view)
      $PokemonSystem.hr_om_party_view = result[:party_view] rescue nil
    end
