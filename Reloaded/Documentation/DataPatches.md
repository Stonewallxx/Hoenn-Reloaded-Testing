...
# Author: Stonewall
#======================================================
# Documents the Reloaded runtime data patch system.
#
# Responsibilities:
...
```ruby
Reloaded::DataPatches.data
```

Read one target:

```ruby
Reloaded::DataPatches.data("example_data")
```

Read one entry:

```ruby
Reloaded::DataPatches.entry("example_data", "example_entry")
```

Register a Reloaded-owned runtime target before patches are rebuilt:
...
```ruby
Reloaded::DataPatches.summary
Reloaded::DataPatches.patches
Reloaded::DataPatches.applied
Reloaded::DataPatches.errors
Reloaded::DataPatches.warnings
```

## Built-In Runtime Targets

Reloaded currently includes these direct runtime targets:
...
`id_number` is optional. If omitted, Reloaded assigns the next available item
...
`id_number` is optional. If omitted, Reloaded assigns the next available move
number at runtime. Advanced mods may provide an explicit `id_number`, but it
...
- `target` is the battle target mode. Common values include `NearOther`, `User`,
  ...
  Reloaded resolves this against `GameData::Target`, so `nearother`,
...
`id_number` is optional. If omitted, Reloaded assigns the next available
...
`base_stats` and `evs` can be partial objects when using `merge`. Reloaded keeps
...
`GameData::Species` entries. Reloaded rebuilds generated prevolution entries
...
trainer datasets, so Reloaded exposes three targets:
...
```json
{
  ...
  "operation": "merge",
  "id": "YOUNGSTER|Reloaded Example|0",
  "data": {
    "bag_items": ["POTION"],
    "battle_text": "Reloaded data patch trainer test!",
    "lose_text": "That was a Reloaded trainer data patch.",
    "trainer_info": "Example trainer created by a Reloaded trainer data patch.",
    "add_pokemon": [
      {
        "species": "poochyena",
        "level": 5,
...
`items` is the base game trainer battle item list. Reloaded also accepts
...
`item` is the base game held item field for a trainer Pokemon. Reloaded also
...
- Species IDs are required for party members. If a trainer Pokemon references an
  unknown species, Reloaded logs an error and skips that party member.
...
Some story battles are not backed by `GameData::Trainer`. For example, Hoenn's
first rival battle is created dynamically in script through a runtime
`BattledTrainer` team. Those battles should be changed with a focused script hook
instead of a trainer data patch.
...
`skill_level` is the base engine AI value. Reloaded also accepts
...
Reloaded also accepts `money` and `reward_money` as clearer aliases. This is
...
`skill_code` is the base engine's compact skill flag string. Reloaded also
...
encounter datasets, so Reloaded exposes three targets:
...
Reloaded also syncs the active `PokemonEncounters` cache after encounter patches
...
```json
{
  ...
  "operation": "add",
  "id": "exampleReloadedOutfit",
  "data": {
    "id": "exampleReloadedOutfit",
    "author": "Stonewall",
    "name": "Example Reloaded Outfit",
...
Shop/mart data is intentionally not part of the data patch bridge. Reloaded
shop behavior will belong to the future Economy/Reloaded Mart system instead.
...
Each valid data patch registers with `Reloaded::Patches`.

Conflict behavior:

- duplicate `add`/`replace` patches for the same target and ID are blocked,
- `replace` patches are treated as critical patch conflicts,
...
After rebuilding, Reloaded emits:
...
Reloaded rebuilds those patches automatically after `GameData.load_all`.
