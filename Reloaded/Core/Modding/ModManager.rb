    ...
    class << self
      ...
      def build_mod_row(mod)
        id = mod[:id].to_s
        {
          ...
          :required_features => Array(mod[:required_features]).map(&:to_s),
      ...
      def read_manifest(manifest_path, root, source)
        ...
        folder_path = File.dirname(manifest_path)
        {
          ...
          :required_features => normalize_string_array(data["required_features"]),
      ...
      def validate_candidate(candidate)
        ...
        if candidate[:manifest].has_key?("required_features") && !candidate[:manifest]["required_features"].is_a?(Array)
          errors << "required_features must be an array"
        end
        ...
        validate_required_features(candidate, errors)
        validate_optional_folders(candidate)
        errors
      end

      def validate_required_features(candidate, errors)
        Array(candidate[:required_features]).each do |feature_id|
          unless defined?(Reloaded::Features) && Reloaded::Features.registered?(feature_id)
            candidate[:system_tags] << "missing_dependency"
            errors << "Requires unknown Reloaded feature: #{feature_id}"
            next
          end
          next if Reloaded::Features.active?(feature_id)
          candidate[:system_tags] << "missing_dependency"
          errors << "Requires Reloaded feature #{feature_id}: #{Reloaded::Features.reason(feature_id)}"
        end
      end
      ...
      def valid_version?(version)
        Reloaded::Versioning.valid?(version)
      end
