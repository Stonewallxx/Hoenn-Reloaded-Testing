#======================================================
# Reloaded Data Patch Trainer Types
# Author: Stonewall
#======================================================
...
#   - Restore Reloaded-managed trainer type entries before each rebuild.
#   - Register the trainer type data patch bridge with Reloaded::Patches.
#
#======================================================
  ...
  module DataPatchTrainerTypes
    ...
    class << self
      def install
        ...
        Reloaded::Log.info("Installed Reloaded trainer type data patch bridge", :mods) if defined?(Reloaded::Log)
        true
      rescue Exception => e
        Reloaded::Log.exception("Trainer type data patch bridge install failed", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def apply_all
        ...
        Reloaded::Log.exception("Failed to apply trainer type data patches", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def apply_entry(id, raw_data)
        ...
        trainer_type = GameData::TrainerType.new(data)
        trainer_type.instance_variable_set(:@reloaded_data_patch, true)
        ...
        Reloaded::Log.exception("Failed to apply trainer type patch #{id}", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def log_applied(count)
        ...
        if defined?(Reloaded::Log)
          if Reloaded::Log.respond_to?(:info_once)
            Reloaded::Log.info_once(message, :mods, key: "trainer_type_data_patch_applied:#{count}")
          else
      ...
      def log_error(message)
        if defined?(Reloaded::Log)
          if Reloaded::Log.respond_to?(:error_once)
            Reloaded::Log.error_once(message, :mods, key: "trainer_type_data_patch_error:#{message}")
          else
      ...
      def register_events
        ...
        Reloaded::Events.on(:game_data_loaded, :trainer_type_data_patch_target_refresh, priority: 50) do |_context|
          Reloaded::DataPatchTrainerTypes.register_target if defined?(Reloaded::DataPatchTrainerTypes)
        end
        Reloaded::Events.on(:data_patches_loaded, :trainer_type_data_patch_bridge, priority: 100) do |_context|
          Reloaded::DataPatchTrainerTypes.apply_all if defined?(Reloaded::DataPatchTrainerTypes)
        end
      end
      ...
      def register_patch_point
        ...
        Reloaded::Patches.register(
          :trainer_type_data_patch_bridge,
          :target => "GameData::TrainerType::DATA",
          :type => :runtime_data_bridge,
          :file => __FILE__,
          :owner => :reloaded,
          :priority => 100,
          :reason => "Applies Reloaded trainer type AI and skill data patches after enabled mods are scanned.",
          :recommended_fix => "Review Reloaded::DataPatchTrainerTypes if patched trainer AI skill does not apply.",
          :conflict_group => "game_data_trainer_types"
        )
      end
    end
  end
...
Reloaded::DataPatchTrainerTypes.install if defined?(Reloaded::DataPatchTrainerTypes)
