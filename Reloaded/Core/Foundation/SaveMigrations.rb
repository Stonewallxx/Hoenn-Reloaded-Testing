#======================================================
# Reloaded Save Migrations
# Author: Stonewall
#======================================================
# Transactional schema migrations for Reloaded and mod save namespaces.
#======================================================
  ...
  module SaveMigrations
    ...
    class << self
      def register(id, from:, to:, &block)
        register_entry(@migrations, :reloaded, id, from, to, block)
      end
      ...
      def migrate(bucket, target_version)
        ...
        applied = []
        begin
          working, applied = run_chain(working, source_version, target, @migrations, :reloaded)
        rescue Exception => e
      ...
      def set_schema_version(hash, version, owner = :reloaded)
        mod_namespace = normalize_owner(owner) != "reloaded"
...
Reloaded::SaveMigrations.register(:reloaded_schema_0_to_1, :from => 0, :to => 1) do |bucket|
