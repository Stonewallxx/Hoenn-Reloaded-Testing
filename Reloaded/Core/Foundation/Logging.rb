  ...
  module Log
    ...
    class << self
      ...
      def report(fields = {})
        ...
        report_key = report_signature(fields, type, level)
        return nil if @report_keys[report_key]
      ...
      def export_bug_report(extra_fields = {}, log_export = true)
        ...
        lines << ""
        lines << "[SYSTEM STATE]"
        lines.concat(system_state_lines)
        lines << ""
        lines << "[VALIDATION]"
        lines.concat(validation_state_lines)
        lines << ""
        lines << "[COUNTS]"
        ...
        lines << "[/BUG REPORT]"
        write_bug_report_file(sanitize_text(lines.join("\n")))
      ...
      def report_signature(fields, type, level)
        values = [type, level]
        [:mod_id, :mod_name, :version, :file_path, :dependency_status, :recommended_fix].each do |key|
          values << (fields[key] || fields[key.to_s]).to_s
        end
        stack = fields[:stack_trace] || fields["stack_trace"]
        values.concat(Array(stack).map { |line| sanitize_text(line) }) if stack
        error = fields[:error] || fields["error"]
        values << error.class.to_s << error.message.to_s if error
        values.map { |value| sanitize_text(value) }.join("|")
      rescue
        [type.to_s, level.to_s, fields.object_id.to_s].join("|")
      end
      ...
      def write_bug_report_file(content)
        text = content.to_s
        text = "[BUG REPORT]\nCould not generate bug report content.\n[/BUG REPORT]" if text.strip.empty?
        tmp_path = "#{BUG_REPORT}.tmp"
        File.open(tmp_path, "w") { |f| f.puts(text) }
        raise "Bug report temp file was empty" if !File.exist?(tmp_path) || File.size(tmp_path).to_i <= 0
        File.rename(tmp_path, BUG_REPORT)
      ensure
        File.delete(tmp_path) rescue nil if defined?(tmp_path) && File.exist?(tmp_path)
      end

      def write_bug_report_fallback(error)
        return if File.exist?(BUG_REPORT) && File.size(BUG_REPORT).to_i > 0
        lines = []
        lines << "[BUG REPORT]"
        lines << "Game Title: #{game_title}"
        lines << "Base Version: #{base_version}"
        ...
        lines << ""
        lines << "[ERRORS / CRITICAL / FATAL]"
        lines << "Bug report generation failed: #{sanitize_text(error.class)}: #{sanitize_text(error.message)}"
        lines << ""
        lines << "[REPORTS]"
        lines << "Could not generate full bug report. Check Log.txt and Mods.txt."
        lines << "[/BUG REPORT]"
      ...
      def system_state_lines
        return ["System registry unavailable."] unless defined?(Reloaded::Systems)
        if defined?(Reloaded::Bootstrap) && Reloaded::Bootstrap.status == :running
          return ["System registration is still in progress during boot."]
        end
        summary = Reloaded::Systems.summary
        lines = [
          "Total: #{summary[:total]}",
        ...
        Reloaded::Systems.systems.each do |system|
          label = system[:name].to_s.empty? ? system[:id].to_s : system[:name].to_s
          line = "- #{label} (#{system[:id]}): #{system[:state].to_s.upcase}"
          line += " - #{system[:reason]}" unless system[:reason].to_s.empty?
          lines << line
        end
        lines
      rescue Exception => e
        ["Could not extract system state: #{e.class}: #{e}"]
      end

      def validation_state_lines
        return ["Validation framework unavailable."] unless defined?(Reloaded::Validation)
        if defined?(Reloaded::Bootstrap) && Reloaded::Bootstrap.status == :running
          return ["Validation is still in progress during boot."]
        end
        summary = Reloaded::Validation.summary
        lines = [
          "Checks: #{summary[:checks]}",
          "Disabled Checks: #{summary[:disabled_checks]}",
          "Warnings: #{summary[:warning]}",
          "Errors: #{summary[:error]}",
          "Critical: #{summary[:critical]}"
        ]
        findings = Reloaded::Validation.results.select do |finding|
          [:warning, :error, :critical].include?(finding[:severity])
        end
        disabled_handlers = defined?(Reloaded::Events) ? Reloaded::Events.disabled_handlers : []
        disabled_handlers.each do |handler|
            lines << "[ERROR] events: Disabled #{handler[:event]}/#{handler[:id]} after #{handler[:failures]} failures."
        end
        if findings.empty? && disabled_handlers.empty?
          lines << "No validation problems found."
        else
          findings.each do |finding|
            lines << "[#{finding[:severity].to_s.upcase}] #{finding[:check]}: #{finding[:message]}"
          end
        end
        lines
      rescue Exception => e
        ["Could not extract validation state: #{e.class}: #{e}"]
      end
      ...
      def parse_manifest_json(path)
        ...
        data.is_a?(Hash) ? stringify_json_keys(data) : {}
      end
      ...
      def stringify_json_keys(value)
        ...
        when Hash
          value.each_with_object({}) do |(key, child), result|
            result[key.to_s] = stringify_json_keys(child)
          end
        when Array
      ...
      def extract_report_blocks
        ...
        text = current_log_text(MAIN_LOG)
      ...
      def extract_severe_log_lines
        ...
        bug_report_log_sources.each do |path, label|
          current_log_lines(path).each do |line|
      ...
      def current_log_text(path)
        current_log_lines(path).join
      rescue
        File.read(path)
      end

      def current_log_lines(path)
        lines = File.readlines(path)
        return lines unless File.expand_path(path) == File.expand_path(MAIN_LOG)
        start_index = nil
        lines.each_with_index do |line, index|
          start_index = index if line.include?("[INFO] Boot start")
        end
        start_index ? lines[start_index..-1] : lines
      rescue
        []
      end
    end
