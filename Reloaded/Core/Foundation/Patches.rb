  ...
  module Patches
    ...
    class << self
      ...
      def registered(target = nil)
        entries = target.nil? ? sorted_patches : sorted_patches.select { |patch| patch[:target] == target.to_s }
        entries.map { |patch| copy_patch(patch) }
      end

      def conflicts(target = nil)
        entries = target.nil? ? sorted_conflicts : @conflicts.select { |conflict| conflict[:target] == target.to_s }
        entries.map { |conflict| copy_conflict(conflict) }
      end
      ...
      def copy_patch(patch)
        copy = patch.dup
        copy[:metadata] = patch[:metadata].dup
        copy
      end

      def copy_conflict(conflict)
        copy = conflict.dup
        copy[:patches] = Array(conflict[:patches]).map(&:dup)
        copy
      end
      ...
      def rebuild_conflicts
        @conflicts.clear
        @conflict_index.clear
        @patches.combination(2) do |first, second|
          next unless related_patches?(first, second) || explicit_conflict?(first, second)
          rule = conflict_between(first, second)
          record_conflict(first, second, rule) if rule
        end
      end

      def related_patches?(patch, other)
        patch[:target] == other[:target] || shared_conflict_group?(patch, other)
      end

      def explicit_conflict?(patch, other)
        conflict_list(patch).include?(patch_identity(other)) ||
          conflict_list(other).include?(patch_identity(patch))
      end
      ...
      def conflict_between(patch, other)
        ...
        return nil unless related_patches?(patch, other)
