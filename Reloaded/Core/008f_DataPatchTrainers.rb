...
#   - Restore Reloaded-managed trainer entries before each rebuild.
...
#   - Register trainer data patch bridge metadata with Reloaded::Patches.
#
#======================================================
  ...
  module DataPatchTrainers
    ...
    class << self
      def install
        ...
        Reloaded::Log.info("Installed Reloaded trainer data patch bridge", :mods) if defined?(Reloaded::Log)
        true
      rescue Exception => e
        Reloaded::Log.exception("Trainer data patch bridge install failed", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def apply_all
        ...
        Reloaded::Log.exception("Failed to apply trainer data patches", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def apply_entry(target, klass, entry_id, raw_data)
        ...
        Reloaded::Log.exception("Failed to apply trainer patch #{target}/#{entry_id}", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end

      def register_trainer(target, klass, data, managed:)
        trainer = klass.new(data)
        trainer.instance_variable_set(:@reloaded_data_patch, true) if managed
      ...
      def trainer_to_hash(trainer)
        {
          ...
          "loseText_rematch" => trainer.loseText_rematch,
          "loseText_rematch_double" => trainer.loseText_rematch_double,
          "battleText" => trainer.battleText,
          "preRematchText" => trainer.preRematchText,
          "preRematchText_caught" => trainer.preRematchText_caught,
          "preRematchText_evolved" => trainer.preRematchText_evolved,
          "preRematchText_fused" => trainer.preRematchText_fused,
          "preRematchText_unfused" => trainer.preRematchText_unfused,
          "preRematchText_reversed" => trainer.preRematchText_reversed,
          "preRematchText_gift" => trainer.preRematchText_gift,
          "infoText" => trainer.infoText,
      ...
      def register_events
        ...
        Reloaded::Events.on(:game_data_loaded, :trainer_data_patch_target_refresh, priority: 50) do |_context|
          if defined?(Reloaded::DataPatchTrainers)
            Reloaded::DataPatchTrainers.send(:refresh_base_entries)
            Reloaded::DataPatchTrainers.register_targets
          end
        end
        Reloaded::Events.on(:data_patches_loaded, :trainer_data_patch_bridge, priority: 100) do |_context|
          Reloaded::DataPatchTrainers.apply_all if defined?(Reloaded::DataPatchTrainers)
        end
      end

      def log_applied(target, count)
        return unless defined?(Reloaded::Log)
        Reloaded::Log.info("Applied #{count} #{target} data patch #{count == 1 ? 'entry' : 'entries'}", :mods)
      end

      def log_error(message)
        if defined?(Reloaded::Log)
          if Reloaded::Log.respond_to?(:error_once)
            Reloaded::Log.error_once(message, :mods, key: "trainer_data_patch_error:#{message}")
          else
      ...
      def log_warning(message)
        if defined?(Reloaded::Log)
          if Reloaded::Log.respond_to?(:warning_once)
            Reloaded::Log.warning_once(message, :mods, key: "trainer_data_patch_warning:#{message}")
          else
            Reloaded::Log.warning(message, :mods)
          end
        end
      end
      ...
      def register_patch_point
        ...
        Reloaded::Patches.register(
          :trainer_data_patch_bridge,
          :target => "GameData::Trainer, TrainerModern, and TrainerExpert party data",
          :type => :runtime_data_bridge,
          :file => __FILE__,
          :owner => :reloaded,
          :priority => 100,
          :reason => "Applies Reloaded trainer party data patches after enabled mods are scanned.",
          :recommended_fix => "Review Reloaded::DataPatchTrainers if patched trainer parties fail to appear.",
          :conflict_group => "game_data_trainers"
        )
      end
    end
  end
...
Reloaded::DataPatchTrainers.install if defined?(Reloaded::DataPatchTrainers)
