  ...
  module Options
    ...
    class << self
      ...
      def set_menu_frame(index)
        ...
        $PokemonSystem.frame = clamp_menu_frame_index(index)
        ensure_options_cursor_contrast!
      ...
      def build_consolidated_master(scene, inloadscreen = false)
        ...
        append_collapsible(master, "VISUALS & UI",
          [
            ...
            take_option(system, "Speech Frame"),
      ...
      def speech_follows_menu_option(scene)
        EnumOption.new(
          ...
          proc { |value|
            $PokemonSystem.reloaded_speech_follows_menu = (value == 1)
            apply_speech_frame
            refresh_option_scene_skins(scene)
          },
...
if defined?(Window_PokemonOption)
  class Window_ReloadedOption < Window_PokemonOption
    ...
    def apply_theme
      theme = Reloaded::Options.theme(($PokemonSystem.reloaded_option_theme rescue 0))
      base, shadow = if Reloaded::Options.current_menu_frame_dark?
                       [theme[:base], theme[:shadow]]
                     else
                       Reloaded::Options.readable_text_colors
                     end
      @baseColor = base
      @shadowColor = shadow
      @nameBaseColor = base
