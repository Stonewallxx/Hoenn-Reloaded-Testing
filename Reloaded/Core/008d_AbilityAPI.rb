...
#   - Register ability behavior changes with Reloaded::Patches.
...
module Reloaded
  module Abilities
    ...
    class << self
      def install
        register_events
        Reloaded::Log.info("Installed Reloaded ability API", :mods) if defined?(Reloaded::Log)
        true
      rescue Exception => e
        Reloaded::Log.exception("Ability API install failed", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def register(id, name: nil, description: nil, id_number: nil)
        ...
        Reloaded::Log.exception("Ability registration failed for #{id}", e, channel: :mods) if defined?(Reloaded::Log)
        nil
      end
      ...
      def on(handler_name, ability_id, &block)
        ...
        register_patch_point(handler_name, id)
        true
      rescue Exception => e
        Reloaded::Log.exception("Ability handler registration failed for #{ability_id}/#{handler_name}", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def copy_behavior(source_ability, *target_abilities)
        ...
        Reloaded::Log.exception("Ability behavior copy failed for #{source_ability}", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def apply_all
        ...
        Reloaded::Log.exception("Ability API reapply failed", e, channel: :mods) if defined?(Reloaded::Log)
        false
      end
      ...
      def register_events
        ...
        Reloaded::Events.on(:data_patches_loaded, :ability_api_reapply, priority: 150) do |_context|
          Reloaded::Abilities.apply_all if defined?(Reloaded::Abilities)
        end
      end

      def register_patch_point(handler_name, ability_id)
        return unless defined?(Reloaded::Patches)
        patch_id = "ability_handler_#{handler_name}_#{ability_id}".gsub(/[^a-zA-Z0-9_]+/, "_").to_sym
        Reloaded::Patches.register(
          patch_id,
          :target => "BattleHandlers::#{handler_name}",
          :type => :ability_behavior,
          :file => caller_locations(2, 1)[0].path,
          :owner => :reloaded,
          :priority => 100,
          :reason => "Registers behavior for ability #{ability_id}.",
          :recommended_fix => "Review mods registering behavior for #{ability_id}.",
          :conflict_group => "ability_behavior:#{ability_id}:#{handler_name}"
        )
      rescue
      end
      ...
      def log_error(message)
        if defined?(Reloaded::Log)
          if Reloaded::Log.respond_to?(:error_once)
            Reloaded::Log.error_once(message, :mods, key: "ability_api_error:#{message}")
          else
...
Reloaded::Abilities.install if defined?(Reloaded::Abilities)
